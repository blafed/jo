...
void jo_echo(const char *fmt, size_t len); //write to output
...
// #define jo_bun_mov(type, val)                                                  \
//   (jo_mov((void **)&(val).ptr),                                                \
//    (jo_bun(type)){(val).ptr, (val).len, (val).flag}, (val).ptr = 0,            \
//    (val).len = 0, (val).flag = 0)
...
// enum JO_STR_FLAG { JO_STR_NULLT = 1, JO_STR_OWN = 2};
